...
(async () => {
  ...
  try {
      ...
      window.chrome = {
        tabs: {
          ...
          sendMessage: async (id, message) => {
            ...
            if (message.type === 'getInvoiceSkus') return { skus: ['SKU-A', 'SKU-B'] };
    ...
    assert.deepEqual(await page.evaluate(() => testCalls.find(x => x.type === 'startBatchMatching')), { type: 'startBatchMatching', pairs: [{ sku: 'SKU-A', purchaseOrder: 'PO-A' }, { sku: 'SKU-B', purchaseOrder: 'PO-B' }], compatibleMode: false });
    await page.evaluate(() => progressListener({
      type: 'batchProgress', message: '本轮结束：已匹配 0 条，未匹配 2 条', level: 'success', done: true,
      report: { phase: 'final', failure: null, results: [
        { status: 'unmatched', sku: 'SKU-A', purchaseOrder: 'PO-A', actualOrders: ['PO-A123'], reason: '兼容匹配未开启' },
        { status: 'unmatched', sku: 'SKU-B', purchaseOrder: 'PO-B', actualOrders: ['PO-B456'], reason: '兼容匹配未开启' },
      ] },
    }, { tab: { id: 1 } }));
    ...
    const compatibleSwitch = page.getByRole('switch', { name: '兼容匹配' });
    await compatibleSwitch.click();
    assert.equal(await compatibleSwitch.getAttribute('aria-checked'), 'true');
    await page.getByRole('button', { name: /开始匹配.*2 行/ }).click();
    await page.waitForFunction(() => testCalls.filter(x => x.type === 'startBatchMatching').length === 2);
    assert.equal(await page.evaluate(() => testCalls.filter(x => x.type === 'startBatchMatching').at(-1).compatibleMode), true);
    assert.equal(await compatibleSwitch.isDisabled(), true, '匹配运行时应锁定兼容匹配开关');
