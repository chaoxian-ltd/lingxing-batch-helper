...
(async () => {
  ...
  try {
      ...
      window.chrome = {
        tabs: {
          ...
          sendMessage: async (id, message) => {
            ...
            if (message.type === 'showBatchReport') return { shown: true };
    ...
    await page.getByRole('button', { name: '查看匹配结果' }).waitFor();
    assert.equal(await page.getByRole('region', { name: '匹配结果' }).count(), 0, '侧栏不应显示匹配结果');
    ...
    assert.equal(await page.evaluate(() => testCalls.at(-1).type), 'showBatchReport');
    await page.evaluate(() => progressListener({ type: 'batchProgress', message: '开始兼容匹配', started: true }, { tab: { id: 1 } }));
    await page.getByRole('button', { name: '暂停匹配' }).waitFor();
    await page.evaluate(() => progressListener({
    ...
    assert.equal(await page.getByRole('button', { name: '查看匹配结果' }).count(), 0, '重新编辑时旧结果应失效');
    await page.waitForFunction(() => testCalls.some(x => x.type === 'clearBatchReport'));
